...
export function builtRuntimeArgs(runtimeRoot: string, args: string[]): string[] {
  return [...ISOLATED_RUNTIME_NODE_ARGS, path.join(runtimeRoot, "dist", "entry.js"), ...args];
}
...
export function runBuiltRuntime(
  ...
  return spawnSync(process.execPath, builtRuntimeArgs(runtimeRoot, args), {
    cwd: runtimeRoot,
    encoding: "utf8",
    env,
    timeout,
